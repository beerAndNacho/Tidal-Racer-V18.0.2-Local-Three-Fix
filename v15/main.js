import { THREE, scene, camera, renderer, composer, configurePost, sunLight, sky, sunDir, water, regionGroups, route, craftModel, animateCraftCharacter, itemBoxes, clouds, resizeEngine } from './engine.js';
...
function updateRace(dt){
  for(const [idx,a] of competitors.entries()){a.t=(a.t+a.speed*dt)%1;const p=route.getPointAt(a.t),tan=route.getTangentAt(a.t),side=new THREE.Vector3(-tan.z,0,tan.x).multiplyScalar(a.lane);a.o.position.copy(p).add(side);const wy=waveHeight(a.o.position.x,a.o.position.z,STATE.time);a.o.position.y=wy+.5;a.o.rotation.y=Math.atan2(tan.x,tan.z);animateCraftCharacter(a.o,{time:STATE.time+idx*.27,speed:a.craft.max*.72,maxSpeed:a.craft.max,steer:Math.sin(STATE.time*.55+idx)*.24,drift:idx%5===0&&Math.sin(STATE.time+idx)>.65,boost:idx%4===0&&Math.sin(STATE.time*.7+idx)>.8,wave:wy*.08})}
  let rank=1;
  if(STATE.mode==='RACE'){
