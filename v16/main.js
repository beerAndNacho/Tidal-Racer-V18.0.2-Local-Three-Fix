import { THREE, scene, camera, renderer, composer, configurePost, sunLight, sky, sunDir, water, updateWaterSurface, regionGroups, route, craftModel, animateCraftCharacter, itemBoxes, clouds, resizeEngine } from './engine.js';
...
import { waveHeight, seaStateFor } from './wave-model.js';
...
function refreshPlayer(){
  ...
  player=craftModel(craft,rider,visualLoadout(),{detail:'hero'});player.position.copy(pos);if(rot)player.rotation.copy(rot);scene.add(player);updateRiderCard(rider);
}
...
refreshPlayer();
const competitors=[];
for(let i=0;i<11;i++){const rr=RIDERS[(i+1)%RIDERS.length],cc=CRAFTS[(i+1)%CRAFTS.length],o=craftModel(cc,rr,{}, {detail:'rival'});scene.add(o);competitors.push({o,rider:rr,craft:cc,t:(i+1)/12,lane:(i%4-1.5)*10,speed:.014+(i%5)*.0013})}
...
function updatePhysics(dt){
  ...
  const steerAuthority=(.42+speedRatio*.66)*(keys.drift?1.32:.82)*craft.turn;
  ...
  lateral+=(steer*speed*.043*(keys.drift?1.85:.58)-lateral*(keys.drift?1.28:3.05))*dt;px+=Math.sin(heading)*speed*dt+Math.cos(heading)*lateral*dt;pz+=Math.cos(heading)*speed*dt-Math.sin(heading)*lateral*dt;
  ...
  player.position.set(px,py,pz);player.rotation.set(pitch,heading,roll);animateCraftCharacter(player,{time:STATE.time,speed,maxSpeed:craft.max,steer,drift:keys.drift,boost:boostOn,wave:roll,vertical,victory:STATE.time<STATE.victoryUntil});emitWake(dt);updateWake(dt);
}
function completeRace(rank){
  ...
  setTimeout(()=>{STATE.lap=1;lastProgress=0;raceT=0;STATE.finished=false;STATE.victoryUntil=0;resetPlayer()},3600);
}
function updateRace(dt){
  for(const [idx,a] of competitors.entries()){a.t=(a.t+a.speed*dt)%1;const p=route.getPointAt(a.t),tan=route.getTangentAt(a.t),side=new THREE.Vector3(-tan.z,0,tan.x).multiplyScalar(a.lane);a.o.position.copy(p).add(side);const wy=waveHeight(a.o.position.x,a.o.position.z,STATE.time,currentSeaState());a.o.position.y=wy+.5;a.o.rotation.y=Math.atan2(tan.x,tan.z);animateCraftCharacter(a.o,{time:STATE.time+idx*.27,speed:a.craft.max*.72,maxSpeed:a.craft.max,steer:Math.sin(STATE.time*.55+idx)*.24,drift:idx%5===0&&Math.sin(STATE.time+idx)>.65,boost:idx%4===0&&Math.sin(STATE.time*.7+idx)>.8,wave:wy*.08})}
  let rank=1;
  if(STATE.mode==='RACE'){
...
function updateWorld(dt){
  ...
  scene.fog.density=lerp(scene.fog.density,storm ? .00074 : .00039,.03);sunLight.intensity=lerp(sunLight.intensity,storm ? 1.65 : 3.15,.035);renderer.toneMappingExposure=lerp(renderer.toneMappingExposure,storm ? .72 : .82,.025);
  const day=(STATE.time*.0025)%1,az=THREE.MathUtils.degToRad(225+day*16);sunDir.setFromSphericalCoords(1,THREE.MathUtils.degToRad(68+Math.sin(day*Math.PI*2)*2.5),az);sky.material.uniforms.sunPosition.value.copy(sunDir);water.material.uniforms.sunDirection.value.copy(sunDir).normalize();
...
function frame(now){
  ...
  }else{player.position.lerp(new THREE.Vector3(1.6,.55,0),.12);player.rotation.y=lerp(player.rotation.y,Math.PI+Math.sin(STATE.time*.22)*.16,.05);player.rotation.x=lerp(player.rotation.x,0,.08);player.rotation.z=lerp(player.rotation.z,0,.08);animateCraftCharacter(player,{time:STATE.time,speed:0,maxSpeed:craft.max,steer:Math.sin(STATE.time*.35)*.08,menu:true,wave:Math.sin(STATE.time*.7)*.08});const orbit=STATE.time*.10;camera.position.lerp(new THREE.Vector3(8.8+Math.sin(orbit)*1.1,4.5,11.8+Math.cos(orbit)*1.1),.04);camera.lookAt(1.2,2.25,.25);updateWaterSurface(STATE.time,.82,player.position.x,player.position.z)}
  composer.render();
}
audioDirector.attach();
window.__tidalV16={STATE,get rider(){return rider},get craft(){return craft},get player(){return player},get heading(){return heading},get speed(){return speed},get seaState(){return currentSeaState()}};
