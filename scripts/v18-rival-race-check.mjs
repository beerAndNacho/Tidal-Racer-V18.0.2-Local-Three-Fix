...
add('main integrates race director with measured route length',main.includes('new RivalRaceDirector({trackLength:route.getLength()')&&main.includes('rivalRace.update(STATE.time,dt'));
add('first-lap route search cannot wrap player a lap ahead',main.includes("rivalRace.playerLap===1&&lastProgress<.12&&raw<0")&&main.includes('const start=route.getPointAt(0)'));
...
add('contact and slipstream affect gameplay',main.includes('rivalRace.registerContact')&&main.includes("audioDirector.cue('impact'")&&main.includes('SLIPSTREAM + BOOST'));
