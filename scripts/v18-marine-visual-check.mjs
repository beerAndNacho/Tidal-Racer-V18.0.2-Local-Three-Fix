...
add('81-fish regional ecology pool',files.engine.includes('for(let i=0;i<81;i++)')&&files.engine.includes('FISH_SPECIES[i%FISH_SPECIES.length]')&&files.engine.includes('d.species.regions.includes(region)')&&files.engine.includes('map:fishTextureFor(species)'));
add('distinct fish anatomy and behavior',files.engine.includes('fishFinMaterials')&&files.engine.includes('rayWingGeometry')&&files.engine.includes('serpentSegmentGeometry')&&files.engine.includes("behavior==='bottom'"));
...
add('large fish remain below surface',files.engine.includes("rarityDepth=['epic','legendary']")&&files.engine.includes('depth:.82+(i%9)*.12+rarityDepth'));
add('opaque close-range fish materials',files.engine.includes('clearcoat:.44')&&files.engine.includes('envMapIntensity:.5')&&!files.engine.includes("opacity:['epic','legendary'].includes(species.rarity)"));
add('species-specific silhouettes',files.engine.includes('rayWhipTail')&&files.engine.includes('opahAnalFin')&&files.engine.includes('grouperJaw')&&files.engine.includes('parrotfishBeak')&&files.engine.includes('tunaFinlet'));
add('breach, fin, wing and serpent animation',files.engine.includes('breachCycle=')&&files.engine.includes('d.pectorals.forEach')&&files.engine.includes('d.wings.forEach')&&files.engine.includes('d.segments.forEach'));
add('three breaching dolphins',files.engine.includes('for(let i=0;i<3;i++)')&&files.engine.includes('breach=active?Math.sin'));
add('live regional marine update export',files.engine.includes("region='GOLDEN COAST'")&&files.main.includes('updateMarineLife(STATE.time,px,pz,sea,speedN,weatherRegion)'));
add('marine browser diagnostics',files.engine.includes('document.body.dataset.marineLife')&&files.engine.includes('dataset.marineVisible')&&files.engine.includes('dataset.marineRegionalSpecies')&&files.engine.includes('dataset.marineBreaches'));
