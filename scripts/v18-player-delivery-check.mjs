...
for(const token of ['new CityDeliveryDirector','cityDelivery:cityDelivery.serialize()','cityDelivery.restore(p.cityDelivery)','function cityDeliveryOfferCard','data-city-delivery-start','function updateCityDelivery','player-courier-checkpoint','deliverySnapshot=cityDelivery.snapshot','tone:\'delivery\'',"event.code!=='KeyJ'"])assert.ok(main.includes(token),'player delivery runtime missing '+token);
...
for(const token of ['id="deliveryHud"','id="deliveryTime"','cityDeliveryOffer','mapJournalCard.delivery'])assert.ok(index.includes(token),'delivery UI missing '+token);
...
for(const [source,token] of [[smoke,'playable player courier contract'],[workflow,'v18-player-delivery-check.mjs'],[readme,'Playable Coast Courier contract'],[contributing,'v18-player-delivery-check.mjs']])assert.ok(source.includes(token),'delivery verification wiring missing '+token);
