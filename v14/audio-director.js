...
class AudioDirector {
  ...
  cue(name,amount=.6){if(typeof document!=='undefined')document.dispatchEvent(new CustomEvent('tidal-audio-cue',{detail:{name}}));if(!this.ctx||!this.unlocked||!this.sfxEnabled)return;const t=this.ctx.currentTime,g=clamp(amount,0,1);
    ...
    switch(name){
      ...
      case'watchAdvisory':sweep(760,520,.16,'triangle',.065);setTimeout(()=>sweep(660,460,.12,'sine',.04),105);break;case'watchIntercept':sweep(390,780,.2,'square',.085);setTimeout(()=>sweep(780,390,.2,'square',.075),210);break;case'watchClear':sweep(420,820,.24,'sine',.065);setTimeout(()=>sweep(620,980,.12,'triangle',.04),125);break;case'watchSettled':sweep(280,920,.3,'triangle',.095);setTimeout(()=>this.cue('uiConfirm',.44),135);break;case'watchInsufficient':sweep(240,92,.25,'square',.065);setTimeout(()=>sweep(210,76,.18,'square',.045),145);break;
